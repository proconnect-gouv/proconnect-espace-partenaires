...
Fonctionnalité: Connexion
  ...
  Scénario: Connexion avec un utilisateur via un lien de connexion envoyé par email puis via ProConnect
    ...
    Quand j'entre la valeur "user@yopmail.com"
    ...
    Quand je clique sur le bouton nommé "Déconnecter user@yopmail.com"
    ...
    Alors je dois voir un titre nommé "Accéder au compte" avec le niveau 1
    Et je clique sur "Renseignez votre mot de passe"
    Quand j'entre la valeur "user@yopmail.com"
    Et je clique sur le bouton nommé "S’identifier"
    Et je clique sur le bouton nommé "Agence nationale de la securite des systemes d'information (ANSSI) (choisir cette organisation)"
    Alors je dois voir un bouton nommé "Déconnecter user@yopmail.com"
  ...
  Scénario: Connexion avec un utilisateur via ProConnect puis via un lien de connexion envoyé par email
    ...
    Alors je dois voir un titre nommé "Vous êtes sur une instance de test" avec le niveau 2
    Et je clique sur "Email professionnel"
    Quand j'entre la valeur "ursula@yopmail.com"
    Et je clique sur le bouton nommé "Continuer"
    Alors je dois voir un titre nommé "Accéder au compte" avec le niveau 1
    Et je clique sur "Renseignez votre mot de passe"
    Quand j'entre la valeur "c1Db*!Jw@s4&2gxb96WZ"
    Et je clique sur le bouton nommé "S’identifier"
    Alors je dois voir un bouton nommé "Déconnecter ursula@yopmail.com"

    Quand je clique sur le bouton nommé "Déconnecter ursula@yopmail.com"
    Alors je dois voir un titre nommé "Rejoignez les partenaires de ProConnect !" avec le niveau 1
