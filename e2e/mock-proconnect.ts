// ProConnect OIDC provider for the e2e suite: its two screens answer the steps
// of uuv/features/login.feature, pick an email then pick a "sub".
...
async function handle(request: IncomingMessage, body: string): Promise<Reply> {
  ...
  switch (url.pathname) {
    ...
    // Step 2 — choose which "sub" logs in.
