...
Cette page décrit les niveaux de confiance eIDAS tels qu'utilisés par ProConnect.

## 1. En bref

ProConnect communique le niveau de confiance d'une authentification via l'attribut `acr` (plus de détails sur cet identifiant technique ici : [qu'est-ce que l'ACR](./../fournisseur-identite/acr-eidas.md)) de trois piliers indépendants : l'**identité** de l'utilisateur, sa méthode d'**authentification**, et le **lien** avec son organisation.

> [!NOTE]
> Les niveaux eidas (`eidas0`, `eidas1`, `eidas1-mfa`, `eidas2`, `eidas3`) sont une interprétation de ProConnect du règlement eIDAS (2015/1502) : le règlement lui-même ne définit pas cette échelle telle quelle.

Pour une vue d'ensemble rapide, consultez le tableau de la partie 2. Pour comprendre en détail un pilier en particulier, rendez-vous directement en partie 3 (identité), 4 (authentification) ou 5 (organisation). Pour le cas particulier d'`eidas0`, voir la partie 6.

## 2. Les niveaux eIDAS : en un coup d'oeil

Chaque niveau est défini selon trois axes :
...
Voici un bref tableau qui donne un aperçu en un coup d'oeil. Tous les détails de chaque colonne seront plus bas dans leurs parties respectives.
...
| Qualité                 | Explication                                                                                                                                                                                                                          | Exemple pour l'identité personnelle                           |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- |
...
| Élevée _(eIDAS)_        | Niveau substantiel, plus : la personne a été identifiée par comparaison de caractéristiques physiques (biométrie ou photographie) auprès d'une source faisant autorité.                                                              | Vérification d'identité avec FranceConnect+ et FranceIdentité |
...
| ------------------------------------------------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mot de passe seul                                                         | eidas1          | Facteur de connaissance unique, pas de protocole cryptographique fort, deux facteurs de catégories différentes sont requis pour atteindre un niveau MFA.                                |
| Code reçu par email (Email OTP)                                           | eidas1-mfa      | L'accès à la messagerie et à l'identité sont réputés différents. La combinaison de ces deux facteurs, d'une même catégorie, permet a titre dérogatoire de fournir un niveau MFA faible. |
| Code reçu par SMS                                                         | eidas1-mfa      | Canal non sécurisé (interception SS7, SIM swapping) : pas un facteur cryptographiquement fort.                                                                                          |
| Application d'authentification (ex. Google Authenticator)                 | eidas2          | Protocole cryptographique fort, mais le secret peut être sauvegardé et transféré sur un autre appareil : seulement _présumé_ sous contrôle exclusif.                                    |
| Notification push (ex. Microsoft Authenticator)                           | eidas2          | Protocole cryptographique fort, mais dépend de la sécurité de l'appareil et du compte cloud associé : seulement _présumée_ sous contrôle exclusif.                                      |
| Clé d'accès (passkey) synchronisée entre appareils (ex. trousseau iCloud) | eidas2          | Protocole cryptographique fort, mais la clé est synchronisée entre appareils : seulement _présumée_ sous contrôle exclusif.                                                             |
| Clé d'accès (passkey) non synchronisée, liée à un seul appareil           | eidas2 à eidas3 | Si la clé est ancrée dans la puce de l'appareil et non exportable, peut atteindre une garantie _fiable_. Dépend de l'implémentation.                                                    |
| Carte à puce professionnelle + code PIN                                   | eidas3          | La clé privée est ancrée dans la puce et ne peut pas être extraite : garantie _fiable_. Résiste aux attaquants à potentiel élevé.                                                       |
| Clé de sécurité physique (ex. YubiKey) + PIN                              | eidas3          | Clé générée dans le secure element, non exportable : garantie _fiable_. Résiste aux attaquants à potentiel élevé.                                                                       |

## 5. Le lien avec l'organisation
...
## 7. Documentation technique associée

Cette page est commune aux deux types de partenaires. Pour l'implémentation technique associée à votre rôle :

- [Fournisseurs de Service : niveaux de confiance eIDAS](../fournisseur-service/niveaux-eidas.md)
