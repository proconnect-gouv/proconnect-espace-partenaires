...
| `claims`        | optionnel        | string                         | `<CLAIMS>` Objet JSON encodé décrivant les claims demandés. Pour récupérer le claim `amr` qui indique le mode d'authentification double facteur utilisé, spécifiez la valeur `{"id_token":{"amr":{"essential":true}}}`. Cf. [quelles sont les valeurs possibles pour le champ amr ?](../ressources/claim_amr.md). Pour exiger un niveau de confiance minimum (eidas), voir [Niveaux eidas](./niveaux-eidas.md). Pour forcer la double authentification (2FA), voir [Double authentification](./double_authentification.md). |
...
| `siret_hint`    | optionnel        | string (14 caractères)         | SIRET de l'organisation à suggérer à l'usager lors de la connexion (cf. [doc](./siret_hint_usage.md))                                                                                                                                                                                                                                                                                                                                                                                                                      |
